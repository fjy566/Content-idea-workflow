...
- 推荐模型：只训练轻量的本地排序模型，可选 XGBoost/CUDA；RTX 4070 Ti Super 可用于训练，但 GPU 不是运行平台的硬依赖。
- 数据源：当前版本只保留中国媒体、中国科技社区以及 `.cn` 域名来源；添加数据源时也会拦截外国域名。
...
首次启动会自动清理数据库中的外国热点数据源，保留文章历史，并补充以下真实公开 RSS：
...
在“数据源”页面可以停用/启用单个来源，也可以点击“补充中国精选源”恢复缺失的精选源。自定义来源必须通过中国域名白名单；平台不会把外国来源偷偷作为回退源。
...
- 有足够反馈样本时优先使用 XGBoost `hist`；检测到 CUDA 时尝试 GPU 训练，失败会记录原因并安全回退 CPU。
...
├─ db.py                   SQLite、WAL、兼容性迁移、启动数据源迁移
├─ models.py               Source/RawItem/Topic/Article/CrawlRun 等模型
├─ source_catalog.py       中国数据源白名单与精选 RSS 目录
...
├─ recommender.py           反馈、训练、GPU/CPU 回退和模型指标
...
如果本机 CUDA、驱动或 XGBoost GPU 轮子不可用，页面会记录 GPU 失败原因并回退 CPU，不影响采集和文章编辑。
...
2. 打开“任务”，勾选本轮要采集的源，点击运行，等待进度到 100%。采集不会自动调用 AI。
3. 回到“热点”，用分类、搜索和分页快速筛选；点进标题查看来源和推荐指标。
...
9. 积累真实收藏、忽略、生成、编辑和发布反馈后，再打开“推荐模型”训练轻量模型。
...
SQLite 开启 WAL 和外键约束。启动时会执行小型兼容迁移：补充新列、回填热点分类、删除外国数据源及其原始采集项、保留文章和无来源历史话题但不再展示到热点雷达。
